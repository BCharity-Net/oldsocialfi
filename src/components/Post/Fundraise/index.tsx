...
export const FUNDRAISE_REVENUE_QUERY = gql`
...
const Fundraise: React.FC<Props> = ({ fund }) => {
  // @ts-ignore
  const collectModule: BCharityCollectModule = fund?.collectModule
  const [showFundersModal, setShowFundersModal] = useState<boolean>(false)
  const { data, loading } = useQuery(FUNDRAISE_REVENUE_QUERY, {
